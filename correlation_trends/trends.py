...
def trend_with_significance(t: ArrayLike, x: ArrayLike, dt: float) -> TrendResult:
    ...
    # residuals, diff between reality and fit
    resid = x - (slope * t + intercept)
    ...
    # the slope in units of its standard error
    n_sigma_raw = slope / se
    n_sigma_eff = slope / se_eff

    # p_value
    p_raw = 2.0 * stats.t.sf(np.abs(n_sigma_raw), df = N - 2)
    p_eff = 2.0 * stats.t.sf(np.abs(n_sigma_eff), df = N_eff - 2)
    ...
    return TrendResult(
        ...
        p_eff = float(p_eff),
        n_eff = float(N_eff),
        t_naive = float(n_sigma_raw),
        t_eff = float(n_sigma_eff),
    )
