...
def calculate_and_plot_trend(amoc_annual_series: xr.DataArray, figsize=(15, 5.5)):
    ...
    if p_value < 0.05:
        ...
        print(
            "The trend did't pass significance testing, it may be a random fluctuation."
        )
    ...
    plt.plot(
        years, y_data, color="crimson", marker="^", linewidth=2, label="Annual Mean"
    )
    plt.plot(
        years,
        trend_line,
...
def plot_mht_timeseries(series: xr.DataArray, mean_val: float, std_val: float) -> None:
    ...
    plt.fill_between(
        ...
        color="#bdc3c7",
        alpha=0.15,
        label=rf"Standard Deviation ($\sigma$ = $\pm${std_val:.4f} PW)",
    )

    # Canvas decorations and styling
    plt.title(
        "Part A: Time Series of MHT at 35°N (CALAFAT 2025)",
        fontsize=12,
        fontweight="bold",
        pad=15,
    )
...
def plot_filtered_comparison(
    raw_series: xr.DataArray, filtered_series: xr.DataArray
) -> None:
    ...
    plt.title(
        "Part B: MHT Structural Variation and Low-Pass Filtering (CALAFAT 2025)",
        fontsize=12,
        fontweight="bold",
        pad=15,
    )
...
def plot_power_spectrum(
    frequencies: np.ndarray, psd_raw: np.ndarray, psd_filt: np.ndarray
) -> None:
    ...
    # Styling and labeling
    plt.title(
        "MHT Power Spectral Density (Welch's Method)",
        fontsize=12,
        fontweight="bold",
        pad=15,
    )
    ...
    plt.ylabel(
        r"Power Spectral Density ($PW^2 / \mathrm{cyc} \cdot \mathrm{yr}^{-1}$)",
        fontsize=10,
        labelpad=8,
    )
...
def apply_tukey_filter(series: xr.DataArray, window_size: int = 12) -> xr.DataArray:
    ...
    series_filtered = df_series.rolling(
        window=window_size, win_type="tukey", center=True
    ).mean()
    ...
    filtered_da.attrs["processing"] = (
        f"Tukey low-pass filtered with window size {window_size}"
    )
...
def compute_spectral_analysis(
    raw_series: xr.DataArray, filtered_series: xr.DataArray, nperseg: int = 36
) -> Dict[str, Any]:
...
def plot_filter_comparison(
    frequencies: np.ndarray, magnitude_tukey: np.ndarray, magnitude_boxcar: np.ndarray
) -> None:
    ...
    plt.title(
        "Part C: Frequency Response Comparison (Tukey vs. Boxcar)",
        fontsize=12,
        fontweight="bold",
        pad=15,
    )
...
def plot_power_spectrum_with_ci(
    frequencies: np.ndarray,
    psd_raw: np.ndarray,
    psd_filt: np.ndarray,
    ...
    plt.yscale("log")
    plt.title(
        f"MHT Power Spectral Density with 95% Confidence Band (DoF $\\approx$ {dof:.1f})",
        fontsize=12,
        fontweight="bold",
        pad=15,
    )
