...
def compute_confidence_interval(
    psd: np.ndarray, nperseg: int = 36, n_total: int = 68, alpha: float = 0.05
...
def test_calculate_annual_mean():
    ...
    da_daily = xr.DataArray(
        data=np.ones(730) * 15.0, dims=["time"], coords={"time": times}
    )
...
def test_compute_spectral_analysis_satisfies_parseval():
    ...
    time_coords = pd.date_range(
        start="2004-01-01", periods=n_points, freq="3ME"
    )  # 3-monthly sampling

    raw_series = xr.DataArray(
        data=rng.standard_normal(n_points), dims=["TIME"], coords={"TIME": time_coords}
    )
    ...
    spec_results = analysis.compute_spectral_analysis(
        raw_series, filtered_series, nperseg=64
    )
    ...
    assert spec_results["integrated_power"] == pytest.approx(
        spec_results["variance"], rel=0.15
    )
...
def test_compute_confidence_interval():
    ...
    lower, upper, dof = analysis.compute_confidence_interval(
        dummy_psd, nperseg=36, n_total=68, alpha=0.05
    )
